...
wb = openpyxl.load_workbook('testcases/徐工融票系统测试用例.xlsx')
# 获取所有的测试用例数据
...
def test_run(suite_name, case_name, case_info_list):
    ...
    for case_info in case_info_list:
        ...
        print(f'type:{type(api_default_param)}')
        if api_default_param != '' and api_default_param != None:
        ...
        if test_params != '' and test_params!=None:
        ...
        if extract_resp != '' and extract_resp != None:
        ...
        if expect_resp != '' and expect_resp != None:
            ...
            for expect_info in expect_resp:
                ...
                expect_res = str(expect_info['expect'])
